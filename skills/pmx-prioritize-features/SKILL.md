---
name: pmx-prioritize-features
description: Prioritize features using impact, effort, confidence, and strategic value.
---

# Prioritize Features

## When to use
- Use when the task clearly matches this PM activity.
- Use when a structured answer is more useful than a generic brainstorm.
- Use when the user needs explicit trade-offs, implications, or next steps.

## Instructions
1. Clarify the goal, user, or decision context.
2. Use the skill-specific lens below to structure the work.
3. Make assumptions explicit instead of hiding them.
4. End with a practical recommendation or next step.

### Skill-specific lens
1. List candidates with user and business context.
2. Assess expected impact and required effort.
3. Check confidence, urgency, and strategic fit.
4. Recommend a ranking and explain the trade-offs.

## Output
- Ranked list
- Scores / rationale
- Quick wins
- Deferrals
- Recommendation
